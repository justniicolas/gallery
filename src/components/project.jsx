  ...
  useEffect(() => {
    ...
    const fetchDataWithRetry = async (url, config, retries = 0) => {
      try {
        ...
        if (error.response && error.response.status === 403 && retries < MAX_RETRY_ATTEMPTS) {
      ...
      try {
        ...
        const reposWithTopics = await Promise.all(
          recentRepos.map(async (repo) => {
            const topicsResponse = await fetchDataWithRetry(
            ...
            const commitsResponse = await fetchDataWithRetry(
            ...
            const contributorsResponse = await fetchDataWithRetry(
